...
void IOmod::writeText(const std::string& msg, const std::string& color, int x, int y) const {
  int textWidth;
  int textHeight;

  SDL_Color colorTxt;

  colorTxt.r = Gamedata::getInstance().getXmlInt(color + "/red");
  colorTxt.g = Gamedata::getInstance().getXmlInt(color + "/green");
  colorTxt.b = Gamedata::getInstance().getXmlInt(color + "/blue");
  colorTxt.a = Gamedata::getInstance().getXmlInt(color + "/alpha");

  SDL_Surface* surface = 
    TTF_RenderText_Solid(font, msg.c_str(), colorTxt);
