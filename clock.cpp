...
Clock::Clock() :
  ...
  timeAtStart(0), 
  timeAtPause(0),
  currTicks(0), prevTicks(0), ticks(0), avgF()
  {
  startClock();
}
...
Clock::Clock(const Clock& c) :
  ...
  currTicks(c.currTicks), prevTicks(c.prevTicks), ticks(c.ticks), avgF(c.avgF)
  {
  startClock();
}
...
int Clock::getAvgFps(){ 
  ...
  int tempfps = getFps();
  int fmax = Gamedata::getInstance().getXmlInt("frameMax");
  if(avgF.size() == 100 ){
    avgF.pop_front();
    avgF.emplace_back(tempfps);
  }
  else{
    avgF.emplace_back(tempfps);
  }
  int sum = 0;
  for(auto i = avgF.begin(); i != avgF.end(); ++i) sum += *i;
  if( sum > 0 ) {
